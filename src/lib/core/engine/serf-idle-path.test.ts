...
describe('IdleOnPath (66) — wake-up logic', () => {
  it('scheduled[rev] set → wakes up into Transporting (3), field_0xe = tick_low+6', () => {
    const f = flag({ index: 1, scheduled: [false, true, false, false, false, false] }); // rev=1 scheduled
    ...
    state.mapTiles[pos].serfIndex = 0; // tile free
    state.serfs[1] = serf;
    dispatchSerf(state, serf);
    expect(serf.state).toBe(3); // Transporting
    expect(serf.stateData[3]).toBe(1); // field_0xe = came = -5+6
    expect(serf.stateData[0]).toBe(0); // field_0xb = 0 (empty)
    expect(state.mapTiles[pos].serfIndex).toBe(1); // tile claimed
    expect(serf.counter).toBe(0);
  });

  it('nothing scheduled → keeps resting (stays 66)', () => {
  ...
  it('the other segment end is scheduled → wakes up, came = oppositeDir(rev)', () => {
  ...
  it('occupied tile -> becomes WaitIdleOnPath (67), not Transporting', () => {
    ...
    const serf = mkSerf({ index: 1, tick: 251, stateData: [1, c0, c1, c2, c3] });
    const pos = posOf(20, 20, geo);
    state.mapTiles[pos].serfIndex = 99; // occupied
    state.serfs[1] = serf;
    dispatchSerf(state, serf);
